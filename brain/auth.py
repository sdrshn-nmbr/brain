...
def authenticate_tailscale_headers(
    ...
    if login:
        ...
        access = AccessLevel.ADMIN if principal in admin_users else AccessLevel.APPEND
    ...
    for rule in rules if isinstance(rules, list) else []:
        ...
        access = {values} if isinstance(values, str) else set(values) if isinstance(values, list) else set()
        if "read" not in access:
            continue
        actor = rule.get("actor", "tailscale-workload")
        if not isinstance(actor, str):
            raise AuthenticationError("Tailscale app capability actor is invalid", 401)
        actor = _clean_principal(actor)
        return Identity(f"workload:{actor}", "tailscale-workload", AccessLevel.READ, actor)
